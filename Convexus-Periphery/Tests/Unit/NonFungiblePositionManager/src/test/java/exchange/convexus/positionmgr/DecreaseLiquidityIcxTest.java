...
public class DecreaseLiquidityIcxTest extends NonFungiblePositionManagerTest {
  ...
  void setup() throws Exception {
    ...
    // create a position
    pool = ConvexusTest.createAndInitializePoolIfNecessary(ConvexusPoolMock.class, alice, factory, ICX.getAddress(), usdc.getAddress(), FEE_AMOUNTS[MEDIUM], encodePriceSqrt(ONE, ONE), tickSpacing);

    final BigInteger hundred = BigInteger.valueOf(100);
    ConvexusLiquidityUtils.depositIcx(alice, nft, hundred);
  ...
  void testDecreasesPositionLiquidity () {
    ...
    BigInteger balance0Before = alice.getBalance();
    BigInteger poolBalance0Before = pool.getAccount().getBalance();
    ...
    BigInteger balance0After = alice.getBalance();
    BigInteger poolBalance0After = pool.getAccount().getBalance();
  ...
  void testCannotDecreaseForMoreThanTheLiquidityOfTheNftPosition () {
    ...
    ConvexusLiquidityUtils.depositIcx(alice, nft, BigInteger.valueOf(200));
